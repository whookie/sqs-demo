...
## A quick note...
 ...
 - As developer for embedded systems, my knowledge in Java-based web applications is severely limited.
...
The project must fulfill the following requirements for a passing grade:
...
Regarding the software, the requirements are:
...
The system is created to allow users to add custom information to geolocation databases,
as users normally don't have access to those databases.
This could be used by a 3rd party service to add custom meta information to an IP-Geolocation
entry, removing the need to spin up a similar service.
Primairly however, the motivation for the system is to get a good grade.
...
| Use Case No.  | 1                                                                     |
| ------------- | --------------------------------------------------------------------- |
| Name          | Adding an information field                                           |
| Precondition  | None                                                                  |
...
| Use Case No.  | 2                                                                     |
| ------------- | --------------------------------------------------------------------- |
| Name          | Deleting an information field                                         |
...
| Use Case No.  | 3                                                                     |
| ------------- | --------------------------------------------------------------------- |
| Name          | Getting the address information                                       |
| Precondition  | Information field added, see Use Case No. 1                           |
...
The project must:
 ...
 - adhere to the original project requirements, mentioned in [Requirements Overview](#requirements-overview)
...
| ------------------------ | ------------------------------------------------------------------- |
| Testability              | Testable code allows for a good test coverage.                      |
| Command Line Application | The project should be able to run on a fully headless server.       |
| Adhere to requirements   | Somewhat obviously, the project should follow defined requirements. |
| Conventions              | The coding convention is the Java default convention.               |
...
The three REST-Endpoints callable by the user are adding or updating information, removing information or requesting information.
The endpoints are reachable under `POST /information/add`, `DELETE /information/delete` and `GET /information/get` respectively.
All endpoints accept the `address` parameter, which takes an IP address as string and references the
IP address the operation is supposed to be executed on.
The `DELETE` and `POST` endpoints additionally take the `name` parameter, used in tandem with the `address` parameter to identify the name of the custom field. Finally, the `POST` endpoint also requires a `value` parameter, used to add or update the value
of the parameter identified by the `address`-`name` combination.

The database is a H2 in-memory database. It is accessed using Java-JPA.

The remote API provides a REST interface. It is accessed using a web request that contains the target IP address in its path. The response is returned as JSON, then parsed accordingly.
